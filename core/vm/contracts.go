...
func (c *getMinerOf) Run(input []byte, evm *EVM) ([]byte, error) {
	...
	if !parsingSuccess {
		log.Error("[Celo] Couldn't parse blocknumber", "input", input)
		return input, fmt.Errorf("Error parsing block number:" + hexutil.Encode(input[0:32]))
	}

	log.Error("[Celo] Run precompiled getMinerOf", "coinbaseByes", hexutil.Encode(input[0:32]), "n", blockNumber.Uint64())
