...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.celo", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/tmptmptmp53451/celo-blockchain/mobile")
