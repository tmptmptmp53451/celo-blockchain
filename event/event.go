...
var reflectTimer = metrics.NewRegisteredTimer("reflectionTimer", nil)
...
func (mux *TypeMux) SubscribeWBuffer(bufferSize int, types ...interface{}) *TypeMuxSubscription {
	sub := newsubWBuffer(mux, bufferSize)
	mux.mutex.Lock()
	defer mux.mutex.Unlock()
	if mux.stopped {
		// set the status to closed so that calling Unsubscribe after this
...
func (mux *TypeMux) PostWOLock(ev interface{}) error {
	...
	rtyp := reflect.TypeOf(ev)
	reflectTimer.UpdateSince(start)
	if mux.stopped {
		return ErrMuxClosed
	}
	subs := mux.subm[rtyp]
...
func newsubWBuffer(mux *TypeMux, bufferSize int) *TypeMuxSubscription {
